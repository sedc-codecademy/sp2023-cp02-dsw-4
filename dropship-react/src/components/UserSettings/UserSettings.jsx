...
function UserSettings() {
  ...
  const showSubmitDiv = () => {
    if (currentPage === 'manage') {
      ...
      if (createCard || !user.cards?.length) return false
    } else {
      return true
    }
  }
  ...
  return (
    <main className="user-settings-main">
      ...
      <div className="dataParent">
        {currentPage === 'accountInfo' ? <AccountInfoFu></AccountInfoFu> : currentPage === 'security' ? <SecurityFu></SecurityFu> : currentPage === 'payment' ? <PaymentFu user={user}></PaymentFu> : <ManageFu></ManageFu>}
        {showSubmitDiv() &&
...
const AccountInfoFu = () => {
  return (
    <div className="dataContainer">
      ...
      <ProfilePicture image={{ alt: 'dime', url: '214124124' }}></ProfilePicture>
  ...
  return (
    <div className="dataContainer">
      ...
      <form className="dataForm paymentForm" onSubmit={(e) => { e.preventDefault() }}>
        {user.cards?.length > 0 && !createCard && (user.cards.map(card => (<SettingsCardHelper key={card.id} cardID={card.id}></SettingsCardHelper>)))}
        {!user.cards?.length > 0 && !createCard && (<div className="noCardFound">
          <NoCardSvg></NoCardSvg>
