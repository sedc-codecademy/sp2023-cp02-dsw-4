...
namespace Dropshiping.BackEnd.Services.UserServices.Validations
{
    public static class CardValidations
    {
        ...
        {
            if (card.CardType != null && (card.CardType < CardTypeEnum.Visa || card.CardType > CardTypeEnum.Default))
            {
                throw new InvalidDataException("Invalid Card Type");
            }
            if (card.CardStatus != null && (card.CardStatus < CardStatusEnum.Primary || card.CardStatus > CardStatusEnum.Secondary))
            {
            ...
            if (card.CardHolder != null && (card.CardHolder.Length > 150 || string.IsNullOrEmpty(card.CardHolder)))
            {
                throw new InvalidDataException("CardHolder canot have more then 150 chars and cannot be empty.");
            }
            if (card.ExpirationDate != null && card.ExpirationDate.Length != 5)
            {
                throw new InvalidDataException("Invalid expiration date");
            }
            if (card.SecurityCode != null && card.SecurityCode.ToString().Length != 3)
            {
